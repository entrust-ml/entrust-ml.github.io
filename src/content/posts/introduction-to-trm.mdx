...
At the time of writing this post, TRM sits at a decent place on the official ARC-AGI-2 leaderboard, reaching 6.3% accuracy at &#0036;2.10 per task. Keep in mind that unlike its big labs' competitors, TRM is trained _from scratch_ and will "limited" compute budget (a few days of 8 H100s).
...
training for each sample to keep track of the number of reasoning steps that this sample went through. The sample is removed from the batch when it has reached the maximum number of steps ($N_sup$) or
