...
The middle of the spectrum tries to compromise between the two extremes. Before embarking on the journey that prioritizes flexibility, we should always ask whether flexibility is worth the price. For us the answer was clear: it is important, not only to be able to quickly add support for additional document types or additional fields to be extracted, but also to be able to do that safely without impacting performance on all already supported documents.
...
with $A \in \mathbb{R}^{r \times d}, B \in \mathbb{R}^{d \times r}$ and $r \ll d$ a hyper-parameter. Then, as the rank of
...
$h=\left(W+\frac{\alpha}{r} A B\right) x$. Empirically, $ \alpha= 2r $, appears to be a good choice, but Sebastian Raschka has [found](https://magazine.sebastianraschka.com/p/practical-tips-for-finetuning-llms) that sometimes tuning alpha can improve performance.
...
$A$ and $B$
 after random initialization and shares them across layers. Instead, VeRA only trains diagonal scaling matrices $\Lambda_d$ and $\Lambda_b$.
